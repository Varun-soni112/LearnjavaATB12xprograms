package Task;

public class task_1 {
//    print Table of 5 by using printf
public static void main(String[] args) {
    int num =5;
    System.out.printf("%d*%d=%d" ,5,1,5*1).println();
    System.out.printf("%d*%d=%d" ,5,2,5*2).println();
    System.out.printf("%d*%d=%d" ,5,3,5*3).println();
    System.out.printf("%d*%d=%d" ,5,4,5*4).println();
    System.out.printf("%d*%d=%d" ,5,5,5*5).println();
    System.out.printf("%d*%d=%d" ,5,6,5*6).println();
    System.out.printf("%d*%d=%d" ,5,7,5*7).println();
    System.out.printf("%d*%d=%d" ,5,8,5*8).println();
    System.out.printf("%d*%d=%d" ,5,9,5*9).println();
    System.out.printf("%d*%d=%d" ,5,10,5*10).println();

}


}
